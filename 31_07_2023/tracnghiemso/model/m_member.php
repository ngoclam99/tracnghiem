...
function Register(
    ...
    if (strlen($cmnd) != 9 and strlen($cmnd) != 12) {
        $msg->icon = "error";
        $msg->statusCode = 500;
        $msg->title = "Vui lòng nhập chứng minh nhân dân: 9 số hoặc mã định danh/CCCD: 12 số";
        $msg->content = mysql_error();
        return $msg;
    }
    ...
    if (mysql_num_rows($result) > 0) {
        $msg->icon = "error";
        $msg->statusCode = 500;
        $msg->title = "Chứng minh nhân dân hoặc mã định danh/CCCD đã bị trùng trên hệ thống vui lòng nhập lại";
        $msg->content = mysql_error();
        return $msg;
    }
