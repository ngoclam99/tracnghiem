...
class CachingXTemplate extends XTemplate {
	...
	protected function write_template_cache () {
		...
		if ($this->cache_expiry > 0 && is_writable($this->cache_dir)) {
			...
			if (!file_exists($this->cache_dir . "/" . $filename)) { // edit by phanliem
				mkdir($this->cache_dir . "/". $filename); // edit by phanliem
			}

			file_put_contents($this->cache_dir . "/" . $filename . "/" . $this->cache_unique . $this->cache_ext, serialize($this->parsed_blocks));
		}
	}
	...
	protected function read_block_cache ($bname, $cache_expiry = 0) {
		...
		$file = $this->cache_dir . "/" . $filename . "/" . $bname . $this->cache_unique . $this->cache_ext;
