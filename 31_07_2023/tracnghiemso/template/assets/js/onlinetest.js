...
$(document).ready(function() {
		...
		$.ajax({
			...
			success: function(json) {
				...
				if( json['template'] )
				{
					...
					$('<div class="summury-info co_dinh text-center ">Thời gian còn lại: <div class="timer" data-seconds-left="1"></div></div>').prependTo('#OnlineTestDoTest .OnlineTestDoTestbox');
	...
	$('#sureOpen_tuluan').on('click', function(){
		
		var type_exam_id = $('#sureOpenTypeExamId').val();
		var token = $('#sureOpenToken').val();
		
		$.ajax({
			url: nv_base_siteurl + 'index.php?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=getquestionwrite&second=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: {type_exam_id : type_exam_id, token : token },
			beforeSend: function() {
				$('#sureOpen_tuluan i').show();
				$('#sureOpen_tuluan').prop('disabled', true);

			},
			complete: function() {
				$('#sureOpen_tuluan i').hide();
				setTimeout(function() {
					$('#sureOpen_tuluan').prop('disabled', false);
				}, 2000);
			},
			success: function(json) {
				if( json['template'] )
				{
					$('#showQuestion').html( json['template'] ).show();
					$('#openTest').remove();
					$('.list-question-number').remove()
					$('.so_cau_da_lam').remove()
					$('#scrolldiv').remove()
					$('.box0').remove()
