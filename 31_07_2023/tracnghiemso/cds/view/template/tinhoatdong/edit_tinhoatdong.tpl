...
<td width="80">  <?php echo $ykxl['tai_khoan'];?> (<?php echo $ykxl['ho_ten'];?>)</td>
</tr>
	    ...
	    <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
	      <table class="form">
			...
			<td><?php echo $ten_tin_hoat_dong; ?></td>
	        	<td><input size="80"  type="text" name="tieu_de" value="<?php echo $tinhoatdong['tieu_de']; ?>"/></td>
		</tr>
	        <tr>
			<td><?php echo $hinhdaidien;?></td>
			<td>
             <input id="xFilePath" name="hinh_anh" type="text" size="60" value="<?php echo $tinhoatdong['hinh_anh']; ?>" />
		    <input type="button" value="Chọn ảnh" onclick="BrowseServer();" /> 
              <?php if($tinhoatdong['file']){ ?>
              <br/>
            <img src ="<?php echo get_thumnai($tinhoatdong['hinh_anh']); ?>" width= "150" height ="112" class="bimg"/> 
              <?php } ?>
            </td>
		</tr>
...
<td>Chuyên mục</td> <td>            
          <select name="dm_id">
              <?php foreach ($list_chuyenmuc as $chuyenmuc){?> 
               <?php if ($chuyenmuc['id'] == $tinhoatdong['dm_id']) { ?>
              <option value="<?php echo $chuyenmuc['id'] ; ?>" selected="selected"><?php echo $chuyenmuc['title']; ?></option>
              <?php } else { ?>
              <option value="<?php echo $chuyenmuc['id']; ?>"><?php echo $chuyenmuc['title']; ?></option>
              <?php } ?>
...
<?php } ?>
			...
			<td><textarea name="mo_ta" cols="150" rows ="10"><?php echo $tinhoatdong['mo_ta']; ?></textarea></td>
	        </tr>
	        <tr>
	        	<td><?php echo $motachitiet;?></td>
	     		<td><textarea name="noi_dung" id="noi_dung"><?php echo $tinhoatdong['noi_dung']; ?></textarea></td>
	        </tr>
	   <tr>
	     <td>Tin tiêu điểm</td>
	     <td><input type="checkbox" name = "noi_bat" value ="1"></td> 
	     </tr>
	      </table>
	    </form>
	  </div>
