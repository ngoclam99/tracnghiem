...
$('.btnSubmitRegister').click(function () {
    ...
    if (cmnd.trim("") == "") {
        $('#msgcmnd').text("Vui lòng nhập chứng minh nhân dân hoặc mã định danh/CCCD!");
        $('#txtcmnd').select();
        return; 
    }

    if (cmnd.trim("").length != 9 && cmnd.trim("").length != 12) {
        $('#msgcmnd').text("Vui lòng nhập chứng minh nhân dân: 9 số hoặc mã định danh/CCCD: 12 số");
        $('#txtcmnd').select();
        return;
    }
    ...
    $.ajax({
        ...
        success: function (count) {
            if (parseInt(count) > 0) {
                $checkcmnnd = 1;
                $('#msgcmnd').text("Chứng minh nhân dân hoặc mã định danh/CCCD đã bị trùng trên hệ thống vui lòng nhập lại");
                $('#txtcmnd').select();
                return false; 
            }  
        }
