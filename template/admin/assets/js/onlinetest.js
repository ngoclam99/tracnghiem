function createEditor_questionwrite(element,key) {
  ...
  CKEDITOR.add;
  $('#getcontent_'+key).val(1)
}
function add_questionwrite(use_ckeditor){
  number_question_write=number_question_write+1;
  var html=''
  html+='<tr class="cau_hoi_tu_luan2" id="questionwrite_tr_'+number_question_write+'">'
  html+='<td class="text-center">'
  html+='<textarea class="form-control" style="width: 100%;height:32px" name="questionwrite['+number_question_write+'][noi_dung]" id="questionwrite_'+number_question_write+'" cols="20" rows="2"></textarea>'
  html+='<div style="float:left;margin-top: 10px;">'
  html+='<label>'
  html+='<a href="javascript:void(0);" onclick="createEditor_questionwrite(\'questionwrite_'+number_question_write+'\','+number_question_write+');">'+use_ckeditor+'</a>'
  html+='</label>'
  html+='<input type="hidden" name="questionwrite['+number_question_write+'][ckeditor]" id="getcontent_'+number_question_write+'" value="0" >'
  html+='</div>'
  html+='</td>'
  html+='<td class="text-center" style="vertical-align: middle;">'
  html+='<button type="button" class="btn btn-danger" onclick="delete_questionwrite('+number_question_write+')">Xóa</button>'
  html+='</td></tr>'
  if($('tr.cau_hoi_tu_luan2').length>0){
    $(html).insertAfter('tr.cau_hoi_tu_luan2:last')
  }else{
    $(html).insertAfter('tr.cau_hoi_tu_luan:last')
  }
}
function delete_questionwrite(key){
  $('#questionwrite_tr_'+key).remove()
}
function check_tu_luan_js(){
  var check=$('#tuluan').is(':checked');
  if(check==true){
    $('.cau_hoi_tu_luan').show()
    $('.cau_hoi_tu_luan2').show()
    $('.cau_hoi_tu_luan3').show()
    $('.checkbox_du_70_diem').show()
  }else{
    $('.cau_hoi_tu_luan').hide()
    $('.cau_hoi_tu_luan2').hide()
    $('.cau_hoi_tu_luan3').hide()
    $('.checkbox_du_70_diem').hide()
  }
}
function  load_static_exam(){
  var exam_id=$('#exam_id').val()
  var data='&mod=load_static_exam&exam_id='+exam_id
  loading()
