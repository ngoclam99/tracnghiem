...
function getListcts($start, $limit)
    {
    $local_list = mysql_query("SELECT * FROM cts  where tinh_trang='0'
    order by
    id_cts desc
    limit ".(int)$start.",".(int)$limit."
    ", dbconnect());
	...
	function getListctsactive($start, $limit)
    {
    $local_list = mysql_query("SELECT * FROM cts  where tinh_trang='1'
    order by
    id_cts desc
    limit ".(int)$start.",".(int)$limit."
    ", dbconnect());
